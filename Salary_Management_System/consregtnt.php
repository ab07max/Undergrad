<?php 
	...
	$year=$_SESSION['year'];
	$i=0;
	$sql="select * from salary where (year='$year' and month='$month' and type='Regular' and (stream='teaching' or stream='nonteaching'))";
...
<?php		
		...
		while($row=mysqli_fetch_array($query,MYSQLI_ASSOC)){
			...
			$sl="select * from deductions_nt where (fid='$fid' and month='$month' and year='$year')";
			$qr=mysqli_query($con,$sl);
			$rw=mysqli_fetch_array($qr,MYSQLI_ASSOC);
			if($row['stream']=='teaching' && $row['type']=='Regular' && $row['month']=="$month" && $row['year']=="$year"){
				$epf=$epf+$ro['epf'];
				$gis=$gis+$ro['gis'];
			...
			else if($row['stream']=='nonteaching' && $row['type']=='Regular' && $row['month']=="$month" && $row['year']=="$year"){
				//echo $fid;
				$epf=$epf+$rw['epf'];
				$gis=$gis+$rw['gis'];
				...
				$quarters=$quarters+$rw['quartersrent'];
				$festival=$festival+$rw['festival'];
				$electricity=$electricity+$rw['electricity'];
...
<tr align="left"><td width="86"></td><td width="218"></td><th  width="177">Particulars<th width="155" align="right">Amount Rs:<td width="171"></td>
...
<tr><td></td><td></td><th align="left">Total:<td><?php echo $total;?>
<tr align="center"><th>SL. NO.<th>Particulars<th>Gross Salary<th>Deductions<th>Net Salary
<tr><td align="center">1<td>Regular Teaching Staff&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(<?php echo $c1;?>)<td><?php echo $gross1;?><td><?php echo $deduction1;?><td><?php echo $net1;?>
<tr><td align="center">2<td>Regular Non Teaching Staff&nbsp;&nbsp;(<?php echo $c2;?>)<td><?php echo $gross2;?><td><?php echo $deduction2;?><td><?php echo $net2;?>
